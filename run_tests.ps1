...
$compileCmd = "vlog -timescale 1ns/1ps -work work -sv adder.v alu.v alu_control.v clk_div.v control_unit.v cu.v data_mem.v imm_ext.v instr_mem.v mux.v pc.v reg_file.v uart_tx.v uart_rx.v uart_regs.v risc_v.v risc_v_isa_tb.v uart_edge_tb.v risc_v_uart_tb.v uart_loopback_tb.v risc_v_uart_full_tb.v"
...
$testbenches = @(
    @{ Name = "RISC-V ISA & Subunit Test"; Module = "risc_v_isa_tb"; Desc = "ALU, Immediates, Control Unit, Register File, Data Memory" },
    @{ Name = "UART Core & MMIO Edge Cases"; Module = "uart_edge_tb"; Desc = "TX/RX patterns, Glitch rejection, Framing errors, MMIO registers" },
    @{ Name = "RISC-V UART Serial Stream"; Module = "risc_v_uart_tb"; Desc = "CPU booting, polling UART MMIO, transmitting serial 'Hi' stream" },
    @{ Name = "UART Hardware Loopback"; Module = "uart_loopback_tb"; Desc = "Direct TX->RX loopback with CPU stream verification" },
    @{ Name = "Full Integration & Recovery"; Module = "risc_v_uart_full_tb"; Desc = "CPU serial TX, external RX injection, and mid-execution reset" }
)
...
foreach ($tb in $testbenches) {
    ...
    $simCmd = "vsim -c -do `"run -all; quit -f`" work.$($tb.Module) -l $logFile"
